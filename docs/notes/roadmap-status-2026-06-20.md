...
| **M3** GC actual freeing | ✅ 1/3-5 chunk-safe close (M3-A) | ADR 0218 着地 (test 1485 → 1488)。chunk-safe programs (no Tables / user fns / TaggedValue locals) で freeing 動作確認。stretch: ADR 0219 (Table DFS)、ADR 0220 (user-fn frame stack walk per ADR 0160) は M3-extended |
