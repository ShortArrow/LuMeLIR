...
**Total:** 29 LIC entries — 29 resolved, 0 partial, 0 pending. Phase 2
...
miss chokepoint without expanding the consumer set.
...
5. **`__newindex` write-path metamethod.** ADR 0134 handles the
   read direction only; `__newindex` requires deciding the rawset
   hatch, aliasing semantics, and whether write-through can re-
   enter `__index`. Deferred per ADR 0133.
