...
| LIC-newindex-function-form-1      | `__newindex = Function` form (Lua §3.4.10) mirror of LIC-index-function-form-1, static-String key + `(Table, String, Number) → ()` void only. Codegen `emit_hash_indexassign_with_newindex` chokepoint else arm extended: when probed `mt["__newindex"]` tag is `TAG_FUNCTION` AND a `(Table, String, Number) → ()` candidate exists, load String key ptr from `key_slot`'s payload (offset 8), dispatch via `emit_dispatch_chain_from_slot_ptr` (ADR 0142 reuse) with `args=[target_ptr, key_ptr, value_v]`. No result; the raw commit+write path is skipped via `handled_by_metatable = true`. Helper signature grows `+functions: &[HirFunction]` threaded through 2 IndexAssign call sites + 1 recursive self-call. Empty candidate set falls through to existing `s_metatable_newindex_unsupported_kind` trap. Table-form arm (ADR 0135) is unchanged. HIR metamethod-aware refinement extended: `__newindex` with `params.len() == 3` (Function form) → `params=[Table, String, Number]`, `ret_kinds=[]`. Number-key Function form, non-Number value, TaggedValue-key Function form, non-void return, multi-hop through Function result deferred | 0082 / 0135 / 0139 / 0141 / 0142 / 0150 / 0151 |
| LIC-string-format-1               | `string.format(fmt_literal, ...)` minimum-scope subset (Lua §6.4). Format string MUST be a static `Str` literal. Supported specifiers: `%d` (Number → integer print), `%f` (Number → float print), `%s` (String → boxed-object data ptr passed to `printf %s`), `%%` (literal `%`). Width / precision / flag suffixes and other specifiers reject HIR-level. HIR parses the format at lower time via `parse_format_specifiers`, validates each non-fmt arg kind against its specifier, enforces arity. Codegen registers the format C-string as `s_format_<hash>` module global, calls `snprintf` into a 256-byte scratch buffer (truncates on overflow), wraps the result via `emit_string_obj_from_bytes` (ADR 0112). 16-arg cap. TaggedValue args, runtime format strings, dynamic scratch growth, full specifier surface deferred | 0103 / 0112 / 0113 / 0152 |
| LIC-pcall-error-strategy-1        | `pcall` / `error` value propagation strategy per ADR 0133 deferral row. Phase 2 = current behaviour preserved (`error(msg)` traps with `s_user_error`, `pcall` rejects at HIR via `UnknownFunction`). Phase 3 = setjmp/longjmp ABI: thread-local `jmp_buf` + error-value slot; `error(msg)` stashes msg and calls `longjmp(jmpbuf, 1)`; `pcall(f, args...)` does `setjmp(jmpbuf)`, dispatches `f` on 0, returns `(false, error-slot)` on 1. Multi-return ABI via ADR 0021 / 0076. String-form error value first cut; Table-form (`error({...})`) bundled with the Phase 3 ADR. Triggers: external user request / patterns ADR / `_ENV` ADR. Decision-only — no Phase 2 code change | 0021 / 0033 / 0076 / 0083 / 0133 / 0142 / 0153 |
| LIC-env-true-globals-strategy-1   | `_ENV` / true globals strategy per ADR 0133 deferral row. Phase 2 = ADR 0048 auto-declared-locals stand-in preserved. Phase 3 = chunk wrapped in implicit `function(_ENV)` that captures `_ENV` upvalue; every bare Ident `x` lowers to `_ENV.x` (read) / `_ENV.x = v` (write); `_ENV` populated at chunk init with `print` / `string.*` / `math.*` / etc.; nested fns inherit `_ENV` via ADR 0083 closure mechanism; `local _ENV = ...` rebinds for lexical scope (Lua §3.5). Builtin migration is dual-arm — HIR early-bind stays + `_ENV.<name>` lookup fallback. Triggers: `_ENV` rebind use case / pcall ADR / patterns ADR / `setfenv` request. Decision-only | 0043 / 0048 / 0083 / 0133 / 0134 / 0145 / 0154 |
| LIC-string-patterns-strategy-1    | `string.find` / `string.match` / `string.gmatch` / `string.gsub` strategy per ADR 0133 deferral row. Phase 2 = HIR rejects (`UnknownFunction`). Phase 3 = port the Lua 5.4 reference pattern matcher (~600 LOC Rust extern). 4 new `Builtin` variants + 4 emit arms. Multi-return ABI via ADR 0021 / 0076 for `find` / `gsub`; iterator ABI via ADR 0085 generic-for for `gmatch`. Character classes (`%a` / `%d` / `%s` / `%w` / `%p` / `%l` / `%u`), sets `[abc]`, ranges, quantifiers (`*` / `+` / `-` / `?`), anchors (`^` / `$`), balanced matches (`%b`), escaped magic. No PCRE/RE2 substitution. Triggers: regex use case / `string.format` widening / `_ENV` ADR. Decision-only | 0021 / 0076 / 0085 / 0103 / 0133 / 0152 / 0155 |
| LIC-gc-architecture-v1-1          | Phase 3 GC architecture v1 elaborating ADR 0145's high-level mark-and-sweep decision into concrete data structures. 16-byte `GcHeader` prefix on every GC-managed allocation: `{u8 mark, u8 type_tag, u16 padding, u32 size, ptr next}`. 7 `GcType` discriminator values (Table=1 / HashBuf=2 / ArrayBuf=3 / StringObj=4 / ClosureCell=5 / UpvalueBox=6 / ScratchBuf=7; ≥100 reserved). Single module-level `g_gc_head` linked list, append-at-head, walked by sweep. Allocator wrapper `emit_gc_alloc(payload_size, type_tag) -> Value<ptr>` returns `raw + 16` (user-visible payload ptr; existing offset constants stay correct). Tri-color mark (WHITE=0 / GREY=1 / BLACK=2) — v1 stop-the-world DFS reaches GREY transiently in the worklist. v1 root set is module globals only — alloca-slot stack walk deferred to v2 (ADR 0160). Mark phase walks type-specific reference graphs (Tables follow array_buf / hash_buf / metatable_ptr; HashBufs walk occupied entries; ClosureCells walk upvalue boxes; etc.). Sweep phase walks `g_gc_head`, frees WHITE nodes, resets BLACK → WHITE. `Builtin::CollectGarbage` (`collectgarbage()` runs a full collection / `collectgarbage("count")` returns heap KB). Implementation roadmap = ADRs 0157-0164 (8 small impl steps). Decision-only | 0083 / 0112 / 0134 / 0145 / 0156 |
| LIC-gc-allocator-wrapper-1        | Phase 3 GC step 1 implementation per ADR 0156's roadmap. New module globals `g_gc_head` (mutable ptr, init null), `g_gc_total_bytes` (mutable i64, init 0), `s_gc_oom` trap message. New helper `emit_gc_alloc(payload_size, type_tag) -> Value<ptr>` allocates `payload + 16`, null-traps via `s_gc_oom`, inits 16-byte header (mark=0, type_tag, padding, size, next=load(g_gc_head)), atomically pushes to head, accumulates bytes into `g_gc_total_bytes`, returns `raw + 16`. 7 GC type-tag constants + `GC_HEADER_SIZE = 16` move to `tagged.rs`. `emit_string_obj_alloc` (ADR 0112 chokepoint for all string-object allocs) migrated to call `emit_gc_alloc(size, GC_TYPE_STRING_OBJ)`. New `Builtin::CollectGarbage` HIR variant (arity (0, 1), ret Number); `collectgarbage()` returns 0.0 (no-op stub); `collectgarbage("count")` returns `g_gc_total_bytes / 1024.0`. Other options reject HIR-level | 0112 / 0145 / 0156 / 0157 |
| LIC-gc-migrate-remaining-types-1  | Phase 3 GC step 2 — 8 allocator sites migrated from `emit_libc_call_ptr("malloc")` to `emit_gc_alloc(size, GC_TYPE_<X>)`. Sites: Table header ctor + Array buf grow + Array buf ctor + HashBuf initial + HashBuf regrow + closure cell + upvalue box + 2 scratch buffers (snprintf path + tostring(Number) path). After migration `g_gc_head` linked list contains every heap-allocated GC-managed object — `collectgarbage("count")` reports the true heap size; ADR 0159 mark phase has full graph coverage. 16-byte header overhead applies to all types now (tiny tables `{}` go from 40 → 56 bytes payload+header). No layout change — header lives BEFORE user-visible ptr | 0083 / 0156 / 0157 / 0158 |
...
| 0151 | 2.6+-newindex-function-form | `__newindex = Function` form (Lua §3.4.10) mirror of ADR 0150, static-String key + `(Table, String, Number) → ()` void. Codegen `emit_hash_indexassign_with_newindex` chokepoint else arm extended: when probed `mt["__newindex"]` tag is `TAG_FUNCTION` AND a `(Table, String, Number) → ()` candidate exists, load String key ptr from `key_slot`'s payload (offset 8), dispatch via `emit_dispatch_chain_from_slot_ptr` (ADR 0142 reuse) with `args=[target_ptr, key_ptr, value_v]`. No result; mark `handled_by_metatable = true` so the raw commit+write path is skipped. Helper signature grows `+functions: &[HirFunction]` threaded through 2 IndexAssign call sites + 1 recursive self-call. Empty candidate set / Nil tag / Table tag fall through to existing arms. HIR metamethod-aware refinement extended: `__newindex` Function form (params.len() == 3) → `params=[Table, String, Number]`, `ret_kinds=[]`. Number-key Function form, non-Number value, TaggedValue-key Function form, non-void return, multi-hop through Function deferred. No new globals. Adds LIC-newindex-function-form-1 to §4. |
| 0152 | 2.7q-stdlib-string-format | `string.format(fmt_literal, ...)` minimum-scope subset (Lua §6.4). HIR `Builtin::StringFormat` variant (variadic). Format string MUST be a static `Str` literal — runtime-computed format strings deferred. Supported specifiers: `%d` (Number → integer), `%f` (Number → float / `%g`-style), `%s` (String → boxed-object data ptr to printf `%s`), `%%` (literal `%`). Width / precision / flags / other specifiers reject as `TypeMismatch` ("unsupported format spec"). HIR parses the format at lower time, validates each non-fmt arg kind against its specifier, enforces arity. Codegen registers the format C-string as `s_format_<hash>` module global, calls `snprintf` into a 256-byte scratch buffer (truncates on overflow), wraps the result via `emit_string_obj_from_bytes` (ADR 0112). 16-arg cap. Up to 6 e2e covering each specifier + arity error. TaggedValue args, runtime format strings, dynamic scratch growth, full specifier surface deferred. Adds LIC-string-format-1 to §4. |
| 0153 | foundational (no phase tag) | `pcall` / `error` value propagation strategy. Phase 2 preserves current behaviour (`error(msg)` traps, `pcall` rejects at HIR). Phase 3 = setjmp/longjmp at C-ABI boundary, thread-local `jmp_buf` + error-value slot. Multi-return ABI via ADR 0021. String-form error first cut; Table-form bundled with Phase 3 ADR. Decision-only — closes the ADR 0133 deferral row. |
| 0154 | foundational (no phase tag) | `_ENV` / true globals strategy. Phase 2 preserves ADR 0048 auto-declared-locals stand-in. Phase 3 = chunk wrapped in implicit `function(_ENV)`; bare Idents lower to `_ENV.x`; nested fns inherit `_ENV` via ADR 0083 closure mechanism; `local _ENV = ...` rebinds for sandboxing. Builtin migration dual-arm (HIR early-bind + `_ENV.<name>` fallback). Decision-only — closes the ADR 0133 deferral row. |
| 0155 | foundational (no phase tag) | `string` patterns strategy. Phase 2 rejects at HIR. Phase 3 = port the Lua 5.4 reference pattern matcher (~600 LOC) as Rust extern, 4 new builtins (`find` / `match` / `gmatch` / `gsub`), multi-return ABI via ADR 0021 for `find` / `gsub`, iterator ABI via ADR 0085 for `gmatch`. No PCRE/RE2 substitution. Decision-only — closes the ADR 0133 deferral row. |
| 0156 | 3.gc-v1 | Phase 3 GC architecture v1 elaborating ADR 0145. Concrete data structures: 16-byte `GcHeader` (mark / type_tag / padding / size / next), 7 `GcType` discriminator values (Table / HashBuf / ArrayBuf / StringObj / ClosureCell / UpvalueBox / ScratchBuf), single global `g_gc_head` linked list, `emit_gc_alloc(size, type_tag) -> ptr` allocator wrapper. Tri-color mark (WHITE / GREY / BLACK) — v1 stop-the-world DFS reaches GREY transiently. Root set: module globals only in v1 (alloca-slot stack walk deferred to v2 implementation ADR). New `Builtin::CollectGarbage` (`collectgarbage()` / `collectgarbage("count")`). Implementation roadmap: ADRs 0157 (allocator wrapper + string-obj migration) → 0158 (remaining types) → 0159 (mark) → 0160 (stack walk) → 0161 (sweep) → 0162 (1 MB trigger) → 0163 (`__gc`) → 0164 (weak tables). Decision-only — no Phase 2 code change. |
| 0157 | 3.gc-v1-step1 | Phase 3 GC step 1 implementation. New module globals `g_gc_head` (mutable ptr, init null), `g_gc_total_bytes` (mutable i64, init 0), `s_gc_oom` trap message. New helper `emit_gc_alloc(payload_size, type_tag) -> Value<ptr>` in `primitive.rs` allocates `payload + GC_HEADER_SIZE`, null-traps via `s_gc_oom`, inits header (mark=0, type_tag, padding, size, next=load(g_gc_head)), pushes to head, accumulates bytes, returns `raw + GC_HEADER_SIZE`. 7 type-tag constants + `GC_HEADER_SIZE = 16` move to `tagged.rs`. `emit_string_obj_alloc` (ADR 0112 chokepoint) migrated to call `emit_gc_alloc(size, GC_TYPE_STRING_OBJ)` — single migration site covers all string-object allocations. New `Builtin::CollectGarbage` HIR variant (arity (0, 1), ret Number). `collectgarbage()` returns 0.0 (no-op stub until mark/sweep land in ADRs 0159+0161). `collectgarbage("count")` returns `g_gc_total_bytes / 1024.0` as Number. Other options reject HIR-level. Mark/sweep/stack walk/auto trigger all deferred. Adds LIC-gc-allocator-wrapper-1 to §4. |
| 0158 | 3.gc-v1-step2 | Phase 3 GC step 2 — migrate the remaining 8 allocator sites to `emit_gc_alloc`. After this ADR `g_gc_head` linked list contains every heap-allocated GC-managed object: Table headers (`GC_TYPE_TABLE`), HashBuf initial + regrow (`GC_TYPE_HASH_BUF`), ArrayBuf grow + ctor (`GC_TYPE_ARRAY_BUF`), closure cells (`GC_TYPE_CLOSURE_CELL`), upvalue boxes (`GC_TYPE_UPVALUE_BOX`), snprintf scratch buffers + tostring(Number) scratch (`GC_TYPE_SCRATCH_BUF`). Each site changes from `emit_libc_call_ptr("malloc", &[size])` to `emit_gc_alloc(size, GC_TYPE_<X>)`. Existing offset constants stay correct because the 16-byte header lives BEFORE the user-visible ptr. `collectgarbage("count")` now reports the true heap size. ADR 0159 (mark phase) gets full graph coverage as a result. Adds LIC-gc-migrate-remaining-types-1 to §4. |
| 0159 | 3.gc-v1-step3 | Phase 3 GC step 3 — mark phase design. Iterative tri-color DFS. WHITE=0 / GREY=1 / BLACK=2. Per-type reference walks: Tables push array_buf/hash_buf/metatable_ptr; ArrayBufs push reference-kind elements; HashBufs push reference-kind keys + values per occupied entry; ClosureCells push upvalue boxes; UpvalueBoxes push reference-kind payload; String/Scratch terminate. Worklist alloca-backed (cap 4096). v1 root set: module globals + closure singletons + static string pool. Top-level + inner-frame alloca slots deferred to ADR 0160. Until 0160 lands, v1 safety mode pre-marks every allocation BLACK so collection is a no-op (matches Phase 2 leak). Decision-only. |
| 0160 | 3.gc-v1-step4 | Phase 3 GC step 4 — alloca-slot stack walk strategy. Per-frame slot registry: each user fn writes a `SlotTable { count, [{slot_addr, kind_tag}; count] }` to a thread-local `g_current_frame_slots` global at entry; caller saves/restores around calls. Mark phase walks via libunwind (`unw_step` up the call stack) reading each frame's slot table. Fallback: conservative `setjmp` scan if libunwind unavailable (false positives leak but don't corrupt). Slot-kind annotation lets the walker skip Number/Bool/Nil slots. Coroutines (future ADR) extend this. Decision-only. |
| 0161 | 3.gc-v1-step5 | Phase 3 GC step 5 — sweep phase design. Linear walk of `g_gc_head` with prev cursor: BLACK → reset to WHITE + advance; WHITE → unlink from list + decrement `g_gc_total_bytes` + `free(raw_ptr)`. The free uses the raw ptr (with GC header), not the user-visible payload. `collectgarbage()` codegen arm calls `emit_gc_mark()` (ADR 0159) → `emit_gc_sweep()` and returns bytes-collected as the delta of `g_gc_total_bytes`. Replaces the 0-arg no-op stub from ADR 0157. Finaliser hook reserved for ADR 0163. Until ADR 0160 wires real roots, ADR 0159's safety mode keeps sweep collecting nothing. Decision-only. |
| 0162 | 3.gc-v1-step6 | Phase 3 GC step 6 — 1 MiB automatic trigger. `emit_gc_alloc` inlines an end-of-body check: if `g_gc_total_bytes + total_size >= g_gc_threshold` then run mark + sweep before incrementing. Threshold stored in mutable module global `g_gc_threshold` (init 1 MiB). Adaptive doubling after sweep: `g_gc_threshold = min(post_sweep_total * 2, 1 GiB)` to avoid thrashing. Future `collectgarbage("setpause", n)` builtin will configure the multiplier. Decision-only. |
| 0163 | 3.gc-v1-step7 | Phase 3 GC step 7 — `__gc` finaliser hook (Lua §2.5.3). Sweep phase extended: for each WHITE Table, probe `mt.__gc`; if `TAG_FUNCTION` and a `(Table) → ()` candidate exists, call before free. Two-pass sweep to handle resurrection safely: (1) collect (obj_ptr, finaliser_fn) pairs and run all finalisers; (2) walk again and free remaining WHITE. Resurrection-prevention bit deferred to v2. `__mode` mutation after `setmetatable` is no-op (documented spec deviation). Decision-only. |
| 0164 | 3.gc-v1-step8 | Phase 3 GC step 8 — weak tables `__mode` (Lua §2.5.2). Weak-mode flags stored in the GC header padding (ADR 0156 reserved): bit 0 = weak keys, bit 1 = weak values. Set at `setmetatable` time by reading `mt.__mode` string. Mark phase change: skip pushing reference-kind keys / values per the flag. Sweep extended: weak-clear pass before WHITE-free, tombstones reference-kind entries pointing at WHITE objects. Ephemeron tables (Lua 5.4 weak-key + value-reachability) deferred. Late `__mode` mutation no-op (documented). Decision-only. |
