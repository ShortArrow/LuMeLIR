...
| LIC-call-metamethod-1             | `__call` metamethod for callable Table (Lua §3.4.10), Function-form + single-Ident receiver only. HIR `lower_call` early arm rewrites `t(args)` to `t.__call(t, args...)` when `t` resolves to a `ValueKind::Table` Local — `self` is the explicit first arg. The rewrite recurses through ADR 0091 IndexCallee → ADR 0082 IndirectDispatch with no new codegen surface. Metamethod-aware refinement extended: `__call` slot forces `params[0] = Table`; subsequent params refine from the rewritten call shape via the standard ADR 0094 walker arm. Multi-segment receiver, variadic candidates, non-Function `__call`, TaggedValue runtime-Table dispatch, and multi-return `__call` deferred to follow-up ADRs | 0082 / 0091 / 0141 / 0146 |
...
| 0146 | 2.6+-call-metamethod | `__call` metamethod (Lua §3.4.10) — `t(args)` where `t` is a Table-kind Local is rewritten at HIR to `t.__call(t, args...)` (explicit `self` as first arg), routing through ADR 0091's IndexCallee + ADR 0082's IndirectDispatch path. Zero codegen surface change. Metamethod-aware refinement extended: `__call` slot forces `params[0] = Table`; the remaining params (variadic-ish) refine from the rewritten call site via the standard ADR 0094 walker arm. Multi-segment receiver (`obj.inner()`), variadic candidates, non-Function `__call`, TaggedValue runtime-Table, and multi-return `__call` deferred. Adds LIC-call-metamethod-1 to §4. |
