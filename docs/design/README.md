...
- [0315 — n5a-coroutine-create](0315-n5a-coroutine-create.md) — N5-A design. Coroutine core as a C file over `<ucontext.h>` (libc-private struct layout makes Rust extern declarations fragile), compiled by build.rs and linked like the bridge object; linux-first with darwin bake-gated; `TAG_COROUTINE = 8`; `Builtin::CoroutineCreate` passing `(fn_ptr, cell_ptr)` via the Newproxy-arm template; 256 KiB fixed stacks pending N8.
- [0314 — gmatch-closure-synthesis](0314-gmatch-closure-synthesis.md) — N4-F-2b/2c. `string.gmatch` desugars at lowering time into three captured locals (s/pat/pos) plus an iterator closure whose body is a reparsed Lua snippet flowing through the standard FunctionExpr pipeline; no Builtin variant, no new codegen closure machinery. Adds the Nil-param call bypass so the protocol's tagged ctl never hits the Number-payload guard. N4-F closed.
- [0313 — n9d-release-artifacts](0313-n9d-release-artifacts.md) — N9-D. Tag-triggered (`v*`) release workflow building `lumelir` per target in the proven CI-lane environments (amd64 image / arm64 image / macos-15 brew, darwin static); artifacts + SHA256SUMS published via `gh` CLI; `workflow_dispatch` bakes the same pipeline into a draft release.
- [0312 — darwin-stdio-symbols](0312-darwin-stdio-symbols.md) — the darwin lane's first codegen catch: generated code declared the libc stdio globals by glibc names, but libSystem exports `__stdoutp`/`__stdinp`. `stdio_symbol()` in `codegen::emit` is the single mapping point, host-selected via `cfg!` since llc targets the host triple.
- [0311 — n9c-darwin-ci](0311-n9c-darwin-ci.md) — N9-C. arm64-darwin lane on Apple Silicon `macos-15` runners: MLIR from the Homebrew `llvm` bottle (melior's documented macOS path) with a fail-fast 22.x version guard; first non-ELF (Mach-O/libSystem) lane. `continue-on-error` until the bake is green; the flip closes N9-C.
- [0310 — chunk-capture-box](0310-chunk-capture-box.md) — N9-E. The arm64 lane's first catch was a latent stack overflow on BOTH arches: chunk-level captured locals kept 1-byte i1 allocas while the box-store helper wrote widened 8-byte values (fn bodies had the heap-box invariant since ADR 0083; the chunk main didn't). Chunk slot map now mirrors fn Step 6.
- [0309 — audit-exceptions](0309-audit-exceptions.md) — cargo-audit gate policy: update when a fix exists (anyhow → 1.0.103 for RUSTSEC-2026-0190); `.cargo/audit.toml` ignore with justification only for no-fix informational advisories (`paste` unmaintained via melior→tblgen, RUSTSEC-2024-0436). Vulnerability-class advisories never ignored.
- [0308 — n9-arm64-ci](0308-n9-arm64-ci.md) — N9-A/B. Native arm64 CI lane on `ubuntu-24.04-arm` (supersedes the cross-compile plan): new Ubuntu + apt.llvm.org arm64 image (`mlir-22-arm64`) + `clippy-and-test-arm64` job, `continue-on-error` until the image bakes. N9-C darwin + N9-D artifacts pinned as follow-ups.
- [0307 — f2-close](0307-f2-close.md) — **Closes F2** at the ADR 0300 audit scope: all identified Lua-semantic deviations fixed (wraparound, >2^53 precision, math.type over BinOp / any-value, and — this ADR — `& | ~` direct i64 beyond 2^53). Coverage boundary documented; generalization breadth (shifts, fn bodies, boundaries, gate removal) pinned as F2-G-1..4.
- [0306 — int-divmod-r1c](0306-int-divmod-r1c.md) — F2-R1-c steps 1-2. `//` and `%` join the integer-tree gate with branch-free floor corrections + `n//0` trap; comparisons (`== ~= < <= > >=`) over int trees emit `cmpi` on i64 (keyed on `int_slot`, not subtype). ADR 0300 headline probe closed: `maxinteger + 1 == mininteger` → `true`. Bitwise / fn bodies / boundaries / gate removal = step 3.
