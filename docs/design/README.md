...
- [0299 — select-desugar](0299-select-desugar.md) — F1-D partial. `select("#", ...)` → `#_va_pack`; `select(n, ...)` → `_va_pack[n]` (single-value truncation per ADR 0228 precedent). Pure HIR desugar in `lower_call` — no Builtin variant, zero plumbing ripple. Literal-args form + negative n + multi-value tail deferred.
