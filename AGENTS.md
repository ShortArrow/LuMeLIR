...
| ‣ 2.5c-full full closures (Plan B) | **Accepted, MLIR feasibility verified, multi-return shape resolved** | ADR 0083 plan landed via Codex pre-review (closure object ptr 一本化 + shared upvalue boxes); Commit 1 in `e6b256f` ships `src/codegen/closure.rs` skeleton. MLIR feasibility spike (2026-05-07, `docs/notes/closure-feasibility.md`): Pattern A1 PASS, A2 FAIL with explicit migration directive. Migration carry-over spike: B1/B2/B4 PASS, B3 FAIL — `func.call` cannot reach `llvm.func`. Multi-return spike (B5, 2026-05-07): direct multi-result not expressible in melior 0.27 API; `!llvm.struct<(...)>` return + `llvm.insertvalue` chain + caller-side `llvm.extractvalue` PASS. Commit 2a scope expands to ~+150 LOC structural rewrite — every multi-return user fn (`__lumelir_next` per ADR 0081, TaggedValue-widened per ADR 0074, `return a, b` per ADR 0021) wraps return as `!llvm.struct<(...)>` in lockstep with `emit_function`'s `LLVMFuncOperationBuilder` flip; 5 `func::call` → `llvm.call` + 1 `func::call_indirect` → `llvm.call` (operand callee) + 3 `func::constant` → `llvm.mlir.addressof` migrate alongside. Behavior 不変 at LLVM IR level (LLVM multi-return is always struct-coded). Spike branch `spike/closure-mlir` retains the proof binaries (commits `976ad8b`, `865f6c6`, `3af1ac7`). Commit 3 (captured-local boxes) and ADR 0087 (closure-as-iter unlock) follow (ADR 0083) |
...
| ‣ 2.6+ tables / metatables | In progress | full closures (ADR 0083 deferred), closure-with-upvalues, TaggedValue arith coerce, hash-key nil completion, methods, metatables — multiple sub-phases |
