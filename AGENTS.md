...
| ‣ 2.5c-full full closures (Plan B) | **Commit 2a landed; Commit 2b/3 pending** | ADR 0083 plan landed via Codex pre-review (closure object ptr 一本化 + shared upvalue boxes); Commit 1 in `e6b256f` ships `src/codegen/closure.rs` skeleton. MLIR feasibility spike (2026-05-07, `docs/notes/closure-feasibility.md`): Pattern A1 PASS, A2 FAIL with explicit migration directive. Migration carry-over spike: B1/B2/B4 PASS, B3 FAIL — `func.call` cannot reach `llvm.func`. Multi-return spike (B5, 2026-05-07): direct multi-result not expressible in melior 0.27 API; `!llvm.struct<(...)>` return + `llvm.insertvalue` chain + caller-side `llvm.extractvalue` PASS. **Commit 2a (2026-05-07)**: `emit_function` / `emit_main` / `emit_lumelir_next_function` flipped to `LLVMFuncOperationBuilder`; multi-return user fns wrap return as `!llvm.struct<(...)>` (4×i64 for `__lumelir_next`, 2×i64 for ADR 0074-widened heterogeneous returns, N for ADR 0021 multi-position). 5 `func::call` → `llvm.call` (callee-attr) + 1 `func::call_indirect` → `llvm.call` (operand-callee) + 3 `func::constant` → `llvm.mlir.addressof` migrate in lockstep. New `callabi.rs` helpers `struct_return_type` / `emit_pack_struct` / `emit_extract_struct_field`. Function-kind param/ret type flips from `!func.func<...>` to `!llvm.ptr`; redundant unrealized_cast bridges removed. 4 IR-shape tests reframed to LLVM-dialect output. 965 tests stay green; LIC unchanged 26/0/3 (Tidy First). Commit 2b (TAG_FUNCTION semantic cutover via `@user_fn_NN_closure` static globals) + Commit 3 (captured-local boxes, resolves LIC-2.6c-tag-hetero-closure-escape-1) + ADR 0087 candidate (closure-as-iter unlock) follow (ADR 0083) |
...
| ‣ 2.6+ tables / metatables | In progress | full closures (ADR 0083 Commit 2b/3 pending), closure-with-upvalues, TaggedValue arith coerce, hash-key nil completion, methods, metatables — multiple sub-phases |
