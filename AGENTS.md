...
| ‣ 2.5c-full full closures (Plan B) | **Commit 2a / 2a-fix / 2b landed; Commit 3 pending** | ADR 0083 plan landed via Codex pre-review; Commit 1 (`e6b256f`) shipped `src/codegen/closure.rs` skeleton. MLIR feasibility spike (2026-05-07): Pattern A1 PASS / B1/B2/B4 PASS / B5b PASS for multi-return struct wrap. **Commit 2a** (`551d51c`): `emit_function` / `emit_main` / `emit_lumelir_next_function` flipped to `LLVMFuncOperationBuilder`; multi-return wraps as `!llvm.struct<(...)>` (4×i64 / 2×i64 / N positions). 5 `func::call` → `llvm.call` + 1 `func::call_indirect` → `llvm.call` (operand-callee) + 3 `func::constant` → `llvm.mlir.addressof` flip. New `callabi.rs` helpers `struct_return_type` / `emit_pack_struct` / `emit_extract_struct_field`. Function-kind param/ret type `!func.func<...>` → `!llvm.ptr`; redundant unrealized_cast bridges removed. 4 IR-shape test assertions reframed. 965/0, LIC 26/0/3. **Commit 2a-fix** (`c81f16b`): Codex review found that the `!llvm.ptr` Function-value erasure stripped MLIR-verifier safety net protecting `Callee::Indirect`'s hardcoded f64 result type — Bool/Nil/String/Table/multi-return user fns through Function-kind parameters miscompiled silently. HIR-level reject (`HirError::IndirectCallNonNumberReturn`, `check_function_arg_ret_kinds` in `lower_call`) restores the safety net (ADR 0075 amend, lifts when ADR 0087 ships). 5 new reject tests, 970/0. Closure.rs header drift (P3) corrected. **Commit 2b**: per-user-fn `@user_fn_NN_closure` static globals via `closure::emit_user_fn_closure_global` (Pattern A1 + `emit_pack_struct`). Producer flip in `HirExprKind::FunctionRef` and known-FuncId `Local` paths (`addressof @<fn_sym>_closure`). Consumer flip in `emit_indirect_dispatch_chain_in_block` and `Callee::Indirect` arm — both insert `closure::emit_load_closure_fn_ptr` before the actual call / candidate compare. Dispatch-chain candidate side stays at raw fn ptr (no double indirection). `tagged.rs` `TAG_FUNCTION` semantics updated; `emit_value_slot_store_function` no longer needs the unrealized_cast bridge (now dropped from the codebase entirely). 3 new IR-shape tests, 973/0. Singleton property (1 fn = 1 global) preserves Lua spec §3.4.4 closure equality. Commit 3 (captured-local boxes) + ADR 0087 candidate (closure-as-iter unlock) follow (ADR 0083) |
...
| ‣ 2.6b-hash-key-nan hash key NaN runtime diagnostic | **Done** | Lua spec §3.4.5 forbids NaN as a table index. Codex Option A (over TaggedValue arith coerce / closure spike). New `s_table_index_nan` global ("table index is NaN") + `emit_table_index_nan_trap_if` / `emit_hash_key_nan_preflight` helpers. NaN preflight inserted at 4 sites: static Number-key IndexAssign / Index arms (before `f2i` / bounds-check), inline `emit_local_init_tagged` Number-key arm (covers `print(t[0/0])`), and `emit_hash_probe_loop` entry — single chokepoint for every TaggedValue-key call site, no per-caller duplication. `cmpf Une self-self` reused from ADR 0077's `emit_tonumber_for_arith` (qNaN/sNaN/±NaN agnostic). 6 new e2e in `tests/phase2_6b_hash_key_nan.rs` (static Number write/read traps, TaggedValue NaN via ADR 0074 widening, regression Number / TaggedValue-string / nil-trap). 959 → 965 green. LIC-2.6b-hash-key-nan-runtime-1 resolved. 26/0/2 (ADR 0086) |
| ‣ 2.6+ tables / metatables | In progress | full closures (ADR 0083 Commit 3 pending), closure-with-upvalues, TaggedValue arith coerce, hash-key nil completion, methods, metatables — multiple sub-phases |
