...
| ‣ 2.5c-full full closures (Plan B) | **Accepted, MLIR feasibility verified** | ADR 0083 plan landed via Codex pre-review (closure object ptr 一本化 + shared upvalue boxes); Commit 1 in `e6b256f` ships `src/codegen/closure.rs` skeleton (layout constants + 8 helper stubs). MLIR feasibility spike landed 2026-05-07 in `docs/notes/closure-feasibility.md` — Pattern A1 (`llvm.func` user fn + `llvm.mlir.addressof` inside `llvm.mlir.global` initializer region) verifies end-to-end and produces the canonical `@closure_singleton = internal global { ptr, i64 } { ptr @user_fn_42, i64 0 }` LLVM IR. Pattern A2 (existing `func.func` user fn) FAILS verify with explicit migration directive — `llvm.mlir.addressof` rejects `func.func` symbols. Commit 2 directive: migrate `emit_function` from `func::func` to `LLVMFuncOperationBuilder` (parallels libc decl emission), then emit `@user_fn_NN_closure` per non-capturing fn and switch TAG_FUNCTION producer/consumer atomically. Commit 3 (captured-local boxes) and ADR 0087 (closure-as-iter unlock) follow (ADR 0083) |
