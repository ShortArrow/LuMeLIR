...
| ‣ 2.5c-full full closures (Plan B) | **Accepted, MLIR feasibility verified, body carry-over resolved** | ADR 0083 plan landed via Codex pre-review (closure object ptr 一本化 + shared upvalue boxes); Commit 1 in `e6b256f` ships `src/codegen/closure.rs` skeleton. MLIR feasibility spike (2026-05-07, `docs/notes/closure-feasibility.md`): Pattern A1 (`llvm.func` user fn + `llvm.mlir.addressof` inside `llvm.mlir.global` initializer) PASS produces the canonical `@closure_singleton = internal global { ptr, i64 } { ptr @user_fn_42, i64 0 }` LLVM IR; Pattern A2 (`func.func` 版) FAIL with explicit migration directive. Migration carry-over spike (2026-05-07): B1 (`arith` in `llvm.func` body) PASS, B2 (`scf.if` ditto) PASS, B3 (`func.call` to `llvm.func`) FAIL — verifier rejects `func.func`-only callee resolution; B4 (`llvm.call` fallback) PASS. Go criteria for Commit 2 are all met: migrate `emit_function` to `LLVMFuncOperationBuilder` AND flip 7 sites of `func::call @user_fn_NN` → `llvm.call` + 3 sites of `func::constant @user_fn_NN` → `llvm.mlir.addressof` in lockstep, then emit `@user_fn_NN_closure` per non-capturing fn and switch TAG_FUNCTION producer/consumer atomically. Spike branch `spike/closure-mlir` retains the proof binaries (commits `976ad8b`, `865f6c6`). Commit 3 (captured-local boxes) and ADR 0087 (closure-as-iter unlock) follow (ADR 0083) |
