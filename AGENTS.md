...
| ‣ 2.7q-stdlib-math Stdlib math.* builtins | **Done** | Codex post-0100 review (6 視点) verdict Refactor → Go with critical: builtin dispatch ONLY when `math` is an UNRESOLVED identifier (user shadowing `local math = ...` MUST respect the user's table per Lua semantics); TDD must include shadowing positive pin + unknown-method negative pin. **Pivots from the ADR 0091-0100 method-axis refinement chain to the stdlib axis**, establishing the pattern for future stdlib expansion. HIR: 3 new `Builtin` variants (`MathSqrt`, `MathFloor`, `MathAbs`) + `Builtin::math_from_method(method)` constructor + arity/name/infer_kind dispatch extensions + ret_kinds (all `[Number]`). `lower_call` entry extended with strict shape predicate `Index{Ident("math"), Str(method)}` AND `resolve("math").is_none()` AND `!function_names.contains_key("math")` AND `Builtin::math_from_method(method) = Some(_)` → dispatch as `Callee::Builtin`. Falls through to existing Index-callee path on any guard miss. New `lower_math_builtin_call` helper validates arity + lowers args + emits `Call{Builtin}`. Codegen: `emit_libm_decls` extended with extern `sqrt(f64)→f64` and `fabs(f64)→f64`. New `emit_libc_call_f64` helper in `primitive.rs`. Builtin emit dispatch arm calls libm extern with f64 arg and returns f64. `src/cli/`, `src/pipeline.rs`, `src/parser/`, `src/lexer/` **zero-diff**; `src/codegen/` +67 LOC bounded. 6 new e2e in `tests/phase2_stdlib_math.rs`: 3 happy Red → Green (sqrt/floor/abs basic) + 1 always-green regression-pin + 1 codex-critical shadowing positive pin (`local math = {}; math.identity(x)` via user's table NOT builtin) + 1 codex-critical unknown-method negative pin (`math.notarealmath(4)` surfaces UndefinedName). 1060 → 1066 green. Other math.* fns (pi/random/sin/cos/log/pow), string.*/table.*/io.* libraries, `local m = math` aliasing remain future work. (ADR 0101) |
